...
function pay(obj) {
	WeixinJSBridge.invoke("getBrandWCPayRequest",
		...
		function(res){
			...
			//寰俊鏀粯鎴愬姛涔嬪悗
			//var url = "/wxpay/index.php?g=Wap&m=Payfree&a=callback_adv&payfree_id="+obj.orderid;
			if(res.err_msg=="get_brand_wcpay_request:ok"){
				var msg_url = "/wxpay/index.php?g=Wap&m=MsgPush&a=index";
				var msg_data = {
					openid: obj.wecha_id,
					order_no: obj.orderid,
					pay_time: obj.timeStamp,
					pay_money: obj.money,
					token: obj.token,
					appid: obj.appId
				};
				$.post(msg_url,msg_data,function(res){},'json');
				//}

				$("#amount").text("");
				if(obj.returnUrl){
					window.location.replace(obj.returnUrl);
				}else{
					WeixinJSBridge.call('closeWindow');
				}
				// window.location.replace(obj.returnUrl);
				//setTimeout("window.location.href = \'"+obj.returnUrl+"\'",2000);
			}else{
				var failNotifyUrl = obj.failNotifyUrl;
				var transStatus = 'fail';
				console.log("bank-寰俊 js api 杩斿洖鍙傛暟",res);
				console.log("bank-鏀粯澶辫触鎾姤閫氱煡鍦板潃",failNotifyUrl);
				console.log("bank-鏀粯澶辫触,鐘舵€�",res.err_msg);
				if(res.err_msg == "get_brand_wcpay_request:cancel"){
					transStatus = 'cancel';
					failNotify(failNotifyUrl,transStatus);
					//var err_msg = "鎮ㄥ彇娑堜簡鏀粯";
				}else if(res.err_msg == "get_brand_wcpay_request:fail"){
					var err_msg = "鏀粯澶辫触閿欒淇℃伅锛�"+res.err_desc;
				}else{
					var err_msg = res.err_msg + res.err_desc;
				}
			}
		});
}

function failNotify(notifyUrl,transStatus){
	if(!notifyUrl){
		return;
	}
	// notifyUrl =  notifyUrl + (notifyUrl.indexOf('?') == -1 ? '?' : '&');
	// notifyUrl =  notifyUrl + "trans_status=" + transStatus;
	var token = $("#token").val();
	$.ajax({
		url:"/wxpay/index.php?g=Wap&m=PayfreeBank&a=cancelPayNotify&fail_notify_url="+encodeURIComponent(notifyUrl)+"&token="+token,
		type:'GET',
		success:function(result){
			console.log("failNotify success",result);
		},
		error:function(result){
			console.log("failNotify error"+result.status);
		}
	});
}
...
function safeCall(a) {
	"undefined" == typeof WeixinJSBridge ? document.addEventListener ? document.addEventListener("WeixinJSBridgeReady", a, !1) : document.attachEvent && (document.attachEvent("WeixinJSBridgeReady", a), document.attachEvent("onWeixinJSBridgeReady", a)) : a(),
		window.WebViewJavascriptBridge ? a() : document.addEventListener("WebViewJavascriptBridgeReady", a, !1)
}
...
function alipayFwc(ali) {
	AlipayJSBridge.call("tradePay",{
		...
		setTimeout(function(){window.paybtnflag = false;}, 1000);
		if(!result){
			alert("鏀粯绯荤粺绻佸繖锛佽绋嶅悗鍐嶈瘯!");
			return false;
		}
		var resultMsgArr = new Array();
		resultMsgArr['8000'] = '鍚庡彴鑾峰彇鏀粯缁撴灉瓒呮椂锛屾殏鏃舵湭鎷垮埌鏀粯缁撴灉';
		resultMsgArr['6004'] = '鏀粯杩囩▼涓綉缁滃嚭閿欙紝鏆傛椂鏈嬁鍒版敮浠樼粨鏋�';
		resultMsgArr['7001'] = '瀹㈡埛绔�-閽卞寘涓蹇嵎鏀粯';
		resultMsgArr['6002']='鏅€氱綉缁滃嚭閿�';
		resultMsgArr['4000']='璁㈠崟鏀粯澶辫触';
		resultMsgArr['99']='鐢ㄦ埛鐐瑰嚮蹇樿瀵嗙爜蹇嵎鐣岄潰閫€鍑�';
		if(result.resultCode =='6001'){//1.鐢ㄦ埛涓€斿彇娑�
			failNotify(ali.failNotifyUrl);
			return false;
		}
		if(result.resultCode =='9000'){//2.鏀粯瀹屾垚璺宠浆
			window.location.href=ali.url2;
			return false;
		}
		if(resultMsgArr.hasOwnProperty(result.resultCode)){//3.鏀粯寮傚父澶勭悊
			alert(resultMsgArr[result.resultCode])
			return false;
		}
		if(!resultMsgArr.hasOwnProperty(result.resultCode)){//4.鏈煡寮傚父澶勭悊
			alert("鍏朵粬鏀粯寮傚父");
			return false;
		}
	});
...
function hb_fq(){
	var radioOne = $("#radio-one").attr("checked");
	var radioTwo = $("#radio-two").attr("checked");
	var radioThree = $("#radio-three").attr("checked");
	console.log(77);
	if(radioTwo){
		return 12;
	}
...
function hb_fq_count(money){
	var hb_fq_seller_percent = $("#amount").attr("hb_fq_seller_percent");
	//1.閲戦淇濈暀4浣嶅皬鏁�
	money=  parseFloat(money).toFixed(2);
	//2.鑾峰彇鑺傜偣
	var radioTwoObj  = $("#radio-two").parent().prev();
	var radioThreeObj  = $("#radio-three").parent().prev();
	//3.12鏈熻祴鍊�
	var moneyRateTwelve,eachTwelve,moneyRateSix,eachSix;
	if(hb_fq_seller_percent ==100){//鍟嗗
		moneyRateSix= moneyRateTwelve = parseFloat(0.00).toFixed(2);//璁＄畻12鏈熸墜缁垂
	}else{
		moneyRateTwelve = Math.floor(money*7.5/12)/100;//璁＄畻12鏈熸墜缁垂
		moneyRateSix= Math.floor(money*4.5/6)/100;
	}
	eachTwelve =parseFloat(Math.floor(money*100/12)/100 + parseFloat(moneyRateTwelve)).toFixed(2);
	radioTwoObj.find(".staging-fg-box-lta").text("楼"+eachTwelve+"脳12鏈�");
	radioTwoObj.find(".staging-fg-box-ltb").text("鎵嬬画璐� 楼"+moneyRateTwelve+"/鏈�");
	//3.6鏈熻祴鍊�
	eachSix = parseFloat(Math.floor(money*100/6)/100 + parseFloat(moneyRateSix)).toFixed(2);
	radioThreeObj .find(".staging-fg-box-lta").text("楼"+eachSix+"脳6鏈�");
	radioThreeObj .find(".staging-fg-box-ltb").text("鎵嬬画璐� 楼"+moneyRateSix+"/鏈�");
}
...
function pay_now(type) {
	...
	if($(".staging").attr("data-fi")==1 && b.payfreemoney>=hbFqMoney && type != 'confirm-to-pay'&&(rate_set_id ==126||rate_set_id ==123||rate_set_id ==0)){
		hb_fq_count(b.payfreemoney);
		$(".staging").show();
		return;
	}
	if(type == 'confirm-to-pay'){
		b.hb_fq = hb_fq();
	}
	if(userAgent.match(/Alipay/i)=="alipay"  && rate_set_id !=126 &&  rate_set_id !=123  ){
		...
		if($("#userId").val() !="")
		{
			b.token = $("#token").val(),
				...
				$.ajax({
					type: "GET",
					url:'/index.php?g=Wap&m=AlipayUserinfo&a=create&online=1',
					data: b,
					dataType: 'json',
			...
			b.token = $("#token").val(),
				...
				$.ajax({
					...
					success: function (response) {
						...
						else if (data.type == "alifwc" || data.type=="alipay_jspay") { // 鏀粯瀹� jspay
							alipayFwc(data);
						}else if(!data.url){
							alert("璺宠浆鏀粯澶辫触!"+data.result_msg);
						}
						else { //鏀粯瀹濇垨浜笢绾夸笅鏀粯
							window.location.replace(data.url);
		...
		b.token = $("#token").val(),
			...
			$.ajax({
				...
				success: function (data) {
					...
					else if (a.type == "alifwc") { // 鏀粯瀹濇湇鍔＄獥
						alipayFwc(a);
					}
					else { //鏀粯瀹濇垨浜笢绾夸笅鏀粯
...
var Zepto = function() {
	...
	return Y.matches = function(a, b) {
		...
		Y.init = function(a, c) {
			var d;
			if (!a) return Y.Z();
			if ("string" == typeof a) if (a = a.trim(), "<" == a[0] && K.test(a)) d = Y.fragment(a, RegExp.$1, c),
				a = null;
			else {
	...
	function(a) {
		...
		b.prototype.insert = function(a, b, c) {
			...
			b.prototype.del = function(a) {
				t = a.text(),
				t && a.text(t.substring(0, t.length - 1)),
				"" == a.text() && $("#pay").removeClass("active"),
					this.showCoupon(a),
					discount()
			},
			...
			b.prototype.submitData = function(a) {
				//澶囨敞鏄惁蹇呭～
				var is_tjlk = $("#wrap").attr("is_tjlk");
				var url = $("#wrap").attr("url");
				var discountmoney =$("#wrap").attr("remark");
				var flag = $("#wrap").attr("flag");
				var wxcard = $("#wrap").attr("wxcard");
				if(is_tjlk==1 && flag==1 && wxcard==1){
					$(document).dialog({
						type: 'confirm',
						style: 'ios',
						titleText: '棰嗗崱鏇翠紭鎯�',
						content: '<div class="confirm-ctt">棰嗗彇浼氬憳鍗″悗 <span class="confirm-cttm">鍙紭鎯�'+discountmoney+'鍏�</span></div>',
						buttonStyle: 'stacked',
						buttons: [
							{
								name: '鐐瑰嚮棰嗗崱',
					...
					var ss = $('#remarks').val();
					var mm = $('#is_remark').val();
					var hotel_remark = $('#hotel_remark').val();
					...
					var name = ''
					var time = ''
					remark_name && (name = `鍏ヤ綇濮撳悕:${remark_name}锛宍)
                    remark_time && (time = `鍏ヤ綇鏃堕棿:${remark_time}锛宍)
					remark_text && (remark = `澶囨敞:${remark_text}`)
					var jointRemark = `${name}${time}${remark}`
					ss = $('#remarks').val(jointRemark)
					"" != a.text() && a.text() > 0 && $("#pay").hasClass("active") && ($("#amt").val((100 * a.text()).toFixed(0)), pay_now())
				}
			},
...
//init(),
	$(".coupon-intro").on("tap",
		function() {
			$(".coupon-rule").show().find(".rule-content").addClass("active")
		}),
	$(".coupon-close").on("tap",
		function() {
			$(".coupon-rule").hide().find(".rule-content").removeClass("active")
		});
